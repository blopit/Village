...
void PanZoomLayer::update(float dt) {
    ...
    if (_touches.size() == 1) {
        _accelerationFactor *= 40 * dt * 0.5f;
    }
...
void PanZoomLayer::onTouchesMoved(const std::vector<Touch *> &touches, Event *event) {
    ...
    if (_touches.size() == 1) {
        ...
        LOG_DEBUG("touch1 %f", touch1->getLocation().x);
        LOG_DEBUG("touch2 %f", touch2->getLocation().x);
        ...
        LOG_DEBUG("curPosLayer %f %f", curPosLayer.x, curPosLayer.y);
        LOG_DEBUG("prevPosLayer %f %f", prevPosLayer.x, prevPosLayer.y);
...
void PanZoomLayer::setPosition(Vec2 position) {
    ...
    if (_panBoundsRect.equals(Rect::ZERO) == false) {
        ...
        LOG_DEBUG("boundBox : origin(%.1f, %.1f), size(%.1f, %.1f)", boundBox.origin.x, boundBox.origin.y, boundBox.size.width, boundBox.size.height);
        ...
        LOG_DEBUG("left,right(%.1f, %.1f), top,bottom(%.1f, %.1f)", left, right, top, bottom);
        ...
        LOG_DEBUG("min(%.1f, %.1f), max(%.1f, %.1f)", min_x, min_y, max_x, max_y);
...
void PanZoomLayer::setScale(float scale) {
    auto q = MIN(MAX(scale, _minScale), _maxScale);
